...
class IsarSeedService {
  static Future<void> seedFoodData(Isar isar, {bool forceUpdate = false}) async {
    ...
    debugPrint('📦 [Isar Seeding] จำนวนอาหารใน Isar ก่อนโหลด: $countBefore รายการ (forceUpdate: $forceUpdate)');
    ...
    try {
      debugPrint('📦 [Isar Seeding] เริ่มโหลดไฟล์ assets/data/food_master.json...');
      // อ่านไฟล์ JSON จาก Assets
      final String fJson = await rootBundle.loadString('assets/data/food_master.json');
      debugPrint('📦 [Isar Seeding] โหลดไฟล์ JSON สำเร็จ ขนาด: ${fJson.length} ตัวอักษร');

      // โยนงานถอดรหัส JSON ไปให้ Background Thread ทำงาน
      final List<dynamic> rawData = await compute((String text) => jsonDecode(text) as List<dynamic>, fJson);
      debugPrint('📦 [Isar Seeding] ถอดรหัส JSON สำเร็จ พบอาหารทั้งหมด: ${rawData.length} รายการ');
      ...
      await isar.writeTxn(() async {
        ...
        debugPrint('📦 [Isar Seeding] กำลังบันทึกข้อมูลอาหาร ${foods.length} รายการลง Isar...');
        await isar.foodItems.putAll(foods);
      });

      final countAfter = await isar.foodItems.count();
      debugPrint('📦 [Isar Seeding] บันทึกข้อมูลอาหารลง Isar สำเร็จ! ปัจจุบันมี: $countAfter รายการ');
    } catch (e, stack) {
      debugPrint('🚨 [Isar Seeding Error] เกิดข้อผิดพลาดในขั้นตอนปั๊มข้อมูล: $e');
      debugPrint('$stack');
    }
  }
}