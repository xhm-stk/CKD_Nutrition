...
class NutrientQuota {
  ...
  bool get isNearLimit  => limit > 0 && percent >= 0.8 && !isOverLimit; // แจ้งเตือนสีส้มที่ 80%
}
...
class QuotaEngine {
  static List<NutrientQuota> calculate({required DailyLog? log, CkdRuleCache? rule}) {
    ...
    final phosphorusLimit = log?.customPhosphorus ?? rule?.phosphorusLimitMg ?? 0;
    ...
    return [
      ...
      NutrientQuota(label: 'ฟอสฟอรัส', unit: 'mg', consumed: log?.totalPhosphorusMg ?? 0, limit: phosphorusLimit),
