...
void main() {
  ...
  test('Comprehensive Supabase RLS Security Audit', () async {
    ...
    final healthA = await client.from('user_health_profiles').insert({
      'user_id': userAId, 'weight_kg': 65.0, 'height_cm': 170.0,
    ...
    final logA = await client.from('daily_logs').insert({
      'user_id': userAId, 'log_date': '2026-06-09',
    }).select();
    ...
    String? mealAId;
    try {
      final mealA = await client.from('meals').insert({
        'log_id': logAId, 'food_id': 'F001', 'food_name': 'ข้าวต้มหมู',
        'quantity_g': 200, 'meal_type': 'breakfast',
        'protein_g': 10.0, 'potassium_mg': 150.0, 'sodium_mg': 300.0,
    ...
    recordResult('Check 1: SELECT profiles ของ User A', s1.isEmpty, 'ผลลัพธ์: $s1');

    // Check 2: user_health_profiles
    final s2 = await client.from('user_health_profiles').select().eq('user_id', userAId);
    recordResult('Check 2: SELECT health ของ User A', s2.isEmpty, 'ผลลัพธ์: $s2');

    // Check 3: daily_logs
    final s3 = await client.from('daily_logs').select().eq('user_id', userAId);
    recordResult('Check 3: SELECT daily_logs ของ User A', s3.isEmpty, 'ผลลัพธ์: $s3');

    // Check 4: meals (ถ้ามี)
    try {
      final s4 = await client.from('meals').select().eq('log_id', logAId);
      recordResult('Check 4: SELECT meals ของ User A', s4.isEmpty, 'ผลลัพธ์: $s4');
    } on PostgrestException catch (e) {
      recordResult('Check 4: SELECT meals ของ User A', true, 'Exception: ${e.message}');
    }
    ...
    try {
      final u1 = await client.from('profiles').update({'display_name': 'HACKED'}).eq('id', userAId).select();
      recordResult('Check 5: UPDATE profiles ของ User A', u1.isEmpty, 'ผลลัพธ์: $u1');
    } on PostgrestException catch (e) {
      recordResult('Check 5: UPDATE profiles ของ User A', true, 'Exception: ${e.message}');
    }

    // Check 6: UPDATE user_health_profiles
    try {
      final u2 = await client.from('user_health_profiles').update({'ckd_stage': 'stage_5'}).eq('user_id', userAId).select();
      recordResult('Check 6: UPDATE health ของ User A', u2.isEmpty, 'ผลลัพธ์: $u2');
    } on PostgrestException catch (e) {
      recordResult('Check 6: UPDATE health ของ User A', true, 'Exception: ${e.message}');
    }

    // Check 7: UPDATE daily_logs
    try {
      final u3 = await client.from('daily_logs').update({'total_protein_g': 9999}).eq('user_id', userAId).select();
      recordResult('Check 7: UPDATE daily_logs ของ User A', u3.isEmpty, 'ผลลัพธ์: $u3');
    } on PostgrestException catch (e) {
      recordResult('Check 7: UPDATE daily_logs ของ User A', true, 'Exception: ${e.message}');
    }
    ...
    try {
      final d1 = await client.from('daily_logs').delete().eq('user_id', userAId).select();
      recordResult('Check 8: DELETE daily_logs ของ User A', d1.isEmpty, 'ผลลัพธ์: $d1');
    } on PostgrestException catch (e) {
      recordResult('Check 8: DELETE daily_logs ของ User A', true, 'Exception: ${e.message}');
    }

    // Check 9: DELETE user_health_profiles ของ User A
    try {
      final d2 = await client.from('user_health_profiles').delete().eq('user_id', userAId).select();
      recordResult('Check 9: DELETE health ของ User A', d2.isEmpty, 'ผลลัพธ์: $d2');
    } on PostgrestException catch (e) {
      recordResult('Check 9: DELETE health ของ User A', true, 'Exception: ${e.message}');
    }

    // Check 10: DELETE profiles ของ User A
    try {
      final d3 = await client.from('profiles').delete().eq('id', userAId).select();
      recordResult('Check 10: DELETE profiles ของ User A', d3.isEmpty, 'ผลลัพธ์: $d3');
    } on PostgrestException catch (e) {
      recordResult('Check 10: DELETE profiles ของ User A', true, 'Exception: ${e.message}');
    }
    ...
    try {
      await client.from('user_health_profiles').insert({
        'user_id': userAId, 'weight_kg': 100.0, 'height_cm': 180.0,
        'gender': 'male', 'ckd_stage': 'stage_5',
      });
      recordResult('Check 11: INSERT health สวมรอยเป็น User A', false, 'ข้อมูลถูกแทรกสำเร็จ!');
    } on PostgrestException catch (e) {
      recordResult('Check 11: INSERT health สวมรอยเป็น User A', true, 'Exception: ${e.message}');
    }

    // Check 12: INSERT daily_logs ด้วย user_id ของ User A
    try {
      await client.from('daily_logs').insert({
        'user_id': userAId, 'log_date': '2026-12-31',
      });
      recordResult('Check 12: INSERT daily_logs สวมรอยเป็น User A', false, 'ข้อมูลถูกแทรกสำเร็จ!');
    } on PostgrestException catch (e) {
      recordResult('Check 12: INSERT daily_logs สวมรอยเป็น User A', true, 'Exception: ${e.message}');
    }
    ...
    try {
      ...
      recordResult('Check 13: SELECT profiles (ไม่ล็อกอิน)', a1.isEmpty, 'ผลลัพธ์: ${a1.length} rows');
    } on PostgrestException catch (e) {
      recordResult('Check 13: SELECT profiles (ไม่ล็อกอิน)', true, 'Exception: ${e.message}');
    }

    // Check 14: SELECT user_health_profiles โดยไม่ล็อกอิน
    try {
      final a2 = await anonClient.from('user_health_profiles').select();
      recordResult('Check 14: SELECT health (ไม่ล็อกอิน)', a2.isEmpty, 'ผลลัพธ์: ${a2.length} rows');
    } on PostgrestException catch (e) {
      recordResult('Check 14: SELECT health (ไม่ล็อกอิน)', true, 'Exception: ${e.message}');
    }

    // Check 15: SELECT daily_logs โดยไม่ล็อกอิน
    try {
      final a3 = await anonClient.from('daily_logs').select();
      recordResult('Check 15: SELECT daily_logs (ไม่ล็อกอิน)', a3.isEmpty, 'ผลลัพธ์: ${a3.length} rows');
    } on PostgrestException catch (e) {
      recordResult('Check 15: SELECT daily_logs (ไม่ล็อกอิน)', true, 'Exception: ${e.message}');
    }
    ...
    try {
      ...
      recordResult('Check 16: INSERT profiles (ไม่ล็อกอิน)', false, 'แทรกข้อมูลสำเร็จ!');
    } on PostgrestException catch (e) {
      recordResult('Check 16: INSERT profiles (ไม่ล็อกอิน)', true, 'Exception: ${e.message}');
    }
    ...
    final verifyProfile = await client.from('profiles').select().eq('id', userAId).maybeSingle();
    recordResult('Check 17: Profile ของ User A ยังคงอยู่', verifyProfile != null);

    // Check 18: Health Profile ยังอยู่ + ค่าไม่ถูกแก้ไข
    final verifyHealth = await client.from('user_health_profiles').select().eq('user_id', userAId).maybeSingle();
    final healthIntact = verifyHealth != null && verifyHealth['ckd_stage'] == 'stage_3a';
    recordResult('Check 18: Health ของ User A ยังอยู่ + ค่าไม่ถูกเปลี่ยน', healthIntact, 'ckd_stage: ${verifyHealth?['ckd_stage']}');

    // Check 19: Daily Log ยังอยู่
    final verifyLog = await client.from('daily_logs').select().eq('user_id', userAId);
    recordResult('Check 19: Daily Log ของ User A ยังคงอยู่', verifyLog.isNotEmpty, '${verifyLog.length} rows');
