...
void main() {
  group('Food Master Database Integrity Tests (156 Menus)', () {
    ...
    test(
      '2. อาหารทั้ง 156 เมนู ต้องไม่มีรหัส (foodId) ซ้ำกันเลย (Uniqueness)',
      () {
        ...
        expect(
          ...
          reason: 'มีรหัส foodId ซ้ำซ้อนกันในฐานข้อมูล',
        );
      },
    );

    test(
      '3. อาหารทั้ง 156 เมนู ต้องมีชื่ออาหารและหมวดหมู่ห้ามเป็นค่าว่าง (Required Fields)',
      () {
        for (var food in foods) {
          expect(
            food.name,
            isNotEmpty,
            reason: 'เมนูรหัส ${food.foodId} ไม่มีชื่ออาหาร',
          );
          expect(
            food.category,
            isNotEmpty,
            reason: 'เมนูรหัส ${food.foodId} (${food.name}) ไม่มีหมวดหมู่',
          );
        }
      },
    );
    ...
    test(
      '4. อาหารทั้ง 156 เมนู ต้องสามารถแปลงค่าโภชนาการเป็นตัวเลขได้อย่างปลอดภัยไม่แครช (Type Safety)',
      () {
        for (var food in foods) {
    ...
    test(
      '5. อาหารทั้ง 156 เมนู ต้องไม่มีค่าโภชนาการติดลบ (No Negative Nutrients)',
      () {
        for (var food in foods) {
          expect(
            food.proteinG,
            greaterThanOrEqualTo(0.0),
            reason: '${food.name} มีโปรตีนติดลบ',
          );
          expect(
            food.sodiumMg,
            greaterThanOrEqualTo(0.0),
            reason: '${food.name} มีโซเดียมติดลบ',
          );
          expect(
            food.potassiumMg,
            greaterThanOrEqualTo(0.0),
            reason: '${food.name} มีโพแทสเซียมติดลบ',
          );
        }
      },
    );
