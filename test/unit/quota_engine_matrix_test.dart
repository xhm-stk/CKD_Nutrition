...
void main() {
  group('QuotaEngine Medical Matrix (KDIGO) Tests', () {
    ...
    final testMatrix = [
      ...
      {'stage': 'stage_3a', 'weight': 50.0, 'multiplier': 0.8, 'expected': 40.0},
      {'stage': 'stage_3b', 'weight': 60.0, 'multiplier': 0.6, 'expected': 36.0},
      {'stage': 'stage_4', 'weight': 40.0, 'multiplier': 0.6, 'expected': 24.0},
      {'stage': 'stage_5', 'weight': 75.0, 'multiplier': 0.6, 'expected': 45.0},
      {'stage': 'stage_5', 'weight': 150.0, 'multiplier': 0.6, 'expected': 90.0},
    ];

    for (var scenario in testMatrix) {
      test('Calculate ${scenario['stage']} with weight ${scenario['weight']}kg', () {
        // จำลองข้อมูลที่ View dashboard_summary คืนค่ากลับมาให้ (Multiplier * Weight)
        final dynamicProteinLimit = (scenario['multiplier'] as double) * (scenario['weight'] as double);
    ...
    test('UI Bound Limit Protection Test', () {
      ...
      expect(proteinQuota.percent, equals(2.5)); // percent can exceed 1.0 to show overdose
    });
  });
}
